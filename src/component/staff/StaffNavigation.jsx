...
function StaffNavigation({ onSignOut }) {
  ...
  const toggleSidebar = () => {
    setCollapsed(prev => {
      const next = !prev;
      localStorage.setItem("staffSidebarCollapsed", next);
      return next;
    });
  };
    ...
    <div className="sn-layout">
      ...
      <aside className={`sn-sidebar${collapsed ? " collapsed" : ""}${mobileOpen ? " mobile-open" : ""}`}>
        ...
        <nav className="sn-nav">
          <div className="sn-nav-section-label">MAIN</div>
          {mainItems.map((item) => (
            <NavLink
              key={item.key}
              to={item.path}
              className={({ isActive }) => `sn-menu-item${isActive ? " active" : ""}`}
              title={collapsed ? item.label : ""}
            >
              <span className="sn-menu-icon"><StaffNavIcon navKey={item.key} /></span>
              <span className="sn-menu-label">{item.label}</span>
            </NavLink>
          ))}

          <div className="sn-nav-section-label" style={{ marginTop: "16px" }}>MANAGEMENT</div>
        ...
        <div className="sn-sidebar-footer">
          <div className="sn-user" title={collapsed ? (currentUser?.name || "") : ""}>
